...
export async function POST(request: NextRequest) {
  try {
    console.log('🧹 Force nettoyage des icônes de catégorie...');
    ...
    // Invalider le cache
    try {
      await fetch(`${process.env.VERCEL_URL ? 'https://' + process.env.VERCEL_URL : 'http://localhost:3000'}/api/cache/invalidate`, { method: 'POST' });
      console.log('✅ Cache invalidé');
    } catch (error) {
      console.error('⚠️ Erreur invalidation cache:', error);
    }
    
    console.log(`✅ Force nettoyage terminé: ${cleanedCount} icônes de catégorie modifiées`);
    
    return NextResponse.json({
      success: true,
      message: `Force nettoyage terminé: ${cleanedCount} icônes de catégorie modifiées`,
      cleanedCount,
      cleanedProducts
    });
    
  } catch (error) {
    console.error('❌ Erreur lors du force nettoyage:', error);
    return NextResponse.json(
