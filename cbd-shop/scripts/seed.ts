...
async function main() {
  ...
  const hashedPassword = await bcrypt.hash('votre_nouveau_mot_de_passe', 12)
  ...
  const info1 = await prisma.information.upsert({
    ...
    create: {
      ...
      content: '<h3>Éditeur du site</h3><p><strong>CBD Shop</strong><br />Adresse : 123 Rue du CBD, 75001 Paris<br />Téléphone : 01 23 45 67 89<br />Email : contact@cbdshop.com</p><h3>Hébergement</h3><p>Vercel Inc.<br />Directeur de publication : CBD Shop</p><h3>Conformité</h3><p>Tous nos produits CBD respectent la législation française en vigueur. Le taux de THC est inférieur à 0,2% conformément à la réglementation européenne.</p>',
      type: 'mentions',
      isActive: true,
    },
  })
  ...
  const info2 = await prisma.information.upsert({
    ...
    create: {
      ...
      content: '<h3>1. Objet</h3><p>Les présentes conditions générales de vente s\'appliquent à toutes les commandes passées sur le site CBD Shop.</p><h3>2. Produits</h3><p>Tous nos produits CBD respectent la législation française en vigueur. Le taux de THC est inférieur à 0,2%.</p><h3>3. Commandes</h3><p>Les commandes sont traitées dans les 24-48h. Nous nous réservons le droit d\'annuler toute commande en cas de problème de stock.</p><h3>4. Livraison</h3><p>La livraison est effectuée par Colissimo. Les frais de port sont offerts à partir de 50€ d\'achat.</p><h3>5. Retours</h3><p>Vous disposez de 14 jours pour retourner un produit non conforme. Les frais de retour sont à votre charge.</p>',
      type: 'cgv',
      isActive: true,
    },
  })
